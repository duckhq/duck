...
pub fn process<'a>(context: &'a mut Context, command: EngineEvent) -> bool {
        ...
        EngineEvent::BuildUpdated(build) => {
            ...
            for observer in context.observers {
                ...
                if let Some(collectors) = &observer.info().collectors {
                    let previous_status = context.observer_status
                        .entry(&observer.info().id)
                        .or_insert(BuildStatus::Unknown);
                    let current_status = context.state.builds.current_status_for_collectors(collectors);
            ...
            propagate_to_observers(&context.observers, &mut || Observation::BuildUpdated(&build));
        }
        EngineEvent::BuildStatusChanged(build) => {
            // Send the BuildUpdated event to all observers.
            propagate_to_observers(&context.observers, &mut || Observation::BuildUpdated(&build));
            // Send the BuildStatusChanged event to all observers.
            propagate_to_observers(&context.observers, &mut || Observation::BuildStatusChanged(&build));
        }
