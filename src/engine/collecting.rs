...
mod tests {
    ...
    fn should_not_send_build_updated_event_if_build_is_known() {
        ...
        // Then
        receiver.try_recv().expect("Channel does not have any events");
    }
    ...
    fn should_send_build_updated_event_if_non_absolute_build_status_change(from: BuildStatus, to: BuildStatus) {
        // Given
        let config = Configuration::empty(&TestVariableProvider::new()).unwrap();
        let (sender, receiver) = channel::<EngineEvent>();
        let context = Context {
    ...
    fn should_send_build_status_changed_event_if_absolute_build_status_change(from: BuildStatus, to: BuildStatus) {
        // Given
        let config = Configuration::empty(&TestVariableProvider::new()).unwrap();
        let (sender, receiver) = channel::<EngineEvent>();
        let context = Context {
